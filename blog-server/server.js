...
var posts = []
for (var i=0; i < 3; i++) {
    posts.push({ Title: `Title of Post ${i}`, Body: `Body of Post ${i}`, Delay: 0})
}
...
app.get('/api/list', function (req, res) {
    // Make a copy of posts that we will modify. We'll 
    let postsPlusFake = [...posts]
    postsPlusFake.push({ Title: "Title of Post 3: Will cause 500", Body: "Body will never be seen"})
    postsPlusFake.push({ Title: "Title of Post 4: Will cause 404", Body: "Body will never be seen"})

    res.json(postsPlusFake)
})
...
app.get('/api/read/:id', function (req, res) {
    ...
    if (postId == NaN) {
        ...
        if (postId >= 0 && postId < posts.length) {
            ...
            // Special case, attempting to get post 3 will cause 500 status
            if (postId == 3) {
                res.status(500)
